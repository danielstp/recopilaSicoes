...
def proord(A, c, s, w, x):
    ...
    for v in range(c, j):
        ...
        for t in range(i):
            A[t] = z[t]
    if ((i == 3 and s == 2) or (i == 4 and s == 3) or (i == 7 and s == 6) or (i == 8 and s == 7) or
            (i == 12 and s == 11) or (i == 15 and s == 14) or (i == 16 and s == 15) or (i == 19 and s == 18) or
            (i == 20 and s == 19)):
        for u in range(i):
            z[u] = 0
            z[u] = A[u]
...
def guardaConvocatorias(datos):
    nombreColumnas = ('CUCE', 'Entidad', 'Modalidad', 'Objeto de Contratación', 'Estado', 'Monto Contrato (Bs)',
                      'Fecha Presentación', 'Fecha Publicación', 'Archivos', 'Formularios', 'Persona contacto',
                      'Garantía', 'CostoPliego', 'ARPC', 'Reunión aclaración', 'Fecha Adjudicación / Desierta',
    ...
    for dato in datos['datos']:
        ...
        try:
            ...
            conv = Convocatoria(cuce=dato[0].strip())
            print("Nuevo")
        conv.entidad = dato[1].strip()
        conv.modalidad = dato[2].strip()
        conv.objetoContratación = dato[3].strip()
        conv.estado = dato[4].strip()
        if dato[5].strip():
            conv.montoContrato = dato[5].strip().replace(',', '.')
        if dato[6].strip():
            conv.fechaPresentación = dt.strptime(dato[6].strip(), '%d/%m/%Y')
        if dato[7].strip():
            conv.fechaPublicación = dt.strptime(dato[7].strip(), '%d/%m/%Y')
        conv.archivos = dato[8].strip()
        conv.formularios = dato[9].strip()
        conv.personaContacto = dato[10].strip()
        if conv.garantía:
            conv.garantía = dato[11].strip()
        if dato[12].strip():
            conv.costoPliego = dato[12].strip().replace(',', '.')
        conv.arpc = dato[13].strip()
        # conv.reuniónAclaración = dato[14].strip()
        if dato[14].strip():
            conv.reuniónAclaración = dt.strptime(dato[14].strip(), '%d/%m/%Y %H:%M')
        if dato[15].strip():
            conv.fechaAdjudicaciónDesierta = dt.strptime(dato[15].strip(), '%d/%m/%y')
        # conv.fechaAdjudicaciónDesierta = dato[15].strip()
        conv.departamento = dato[16].strip()
        conv.normativa = dato[17].strip()
        conv.save()
...
def convBase(operacion, tipo, pag):
    ...
    if len(b53) < 4:
        try:
            index = sesion.get("https://www.sicoes.gob.bo/portal/contrataciones/busqueda/convocatorias.php?tipo=" +
                               "convNacional")
        except (requests.exceptions.ConnectionError, requests.exceptions.ChunkedEncodingError):
            index = sesion.get("https://www.sicoes.gob.bo/portal/contrataciones/busqueda/convocatorias.php?tipo=" +
                               "convNacional")
    ...
    try:
        ...
        datos = getDatos(columnas=r[1], ruta=r[0], operacion=operacion, tipo=tipo, sesion=sesion, captcha=r[2], pag=pag,
                         b9=b53)
    except requests.exceptions.ConnectionError:
        r = getRutaColumnasContrataciones(operacion=operacion, tipo=tipo, sesion=sesion)
        datos = getDatos(columnas=r[1], ruta=r[0], operacion=operacion, tipo=tipo, sesion=sesion, captcha=r[2], pag=pag,
...
def convNacional():
    ...
    while pag <= total:
        # datos = convBase(operacion=operacion, tipo=tipo, pag=pag)
        threading.Thread(target=convBase, kwargs={'operacion': operacion, 'tipo': tipo, 'pag': pag}).start()
        pag = pag + 1
        while len(threading.enumerate()) > 10:
            time.sleep(1)


def convInternacional():
    tipo = 'Avanzada'
    operacion = 'convInternacional'
    pag = 1
    datos = []
    datos = convBase(operacion=operacion, tipo=tipo, pag=pag)
    total = int(datos['recordsTotal'])/10
    ...
    while pag <= total:
        ...
        threading.Thread(target=convBase, kwargs={'operacion': operacion, 'tipo': tipo, 'pag': pag}).start()
        pag = pag + 1
        while len(threading.enumerate()) > 60:
            time.sleep(1)


# 'CUCE', 'Entidad', 'Modalidad', 'Objeto de Contratación', 'Estado', 'Monto Contrato (Bs)', 'Fecha Presentación',
# 'Fecha Publicación', 'Archivos', 'Formularios', 'Persona contacto', 'Garantía', 'CostoPliego', 'ARPC',
# 'Reunión aclaración', 'Fecha Adjudicación / Desierta', 'Departamento', 'Normativa'
